...
namespace Gigaspaces.Rest
{
    class RestCommand : Command
    {
        ...
        private Regex ServiceManagementPattern = new Regex("https://management.core.windows.net/[A-Fa-f0-9]{8}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{12}[^\\s]*", OPTIONS);
        private Regex StoragePattern = new Regex("http://([^.]+).(queue|table|blob).core.windows.net[^\\s]*", OPTIONS);
        ...
        public override bool TestOptionSet()
        {
            ...
            else if (type == RequestTarget.Storage)
            {
                ...
                Match m = StoragePattern.Match(url);

                storageAccount = m.Groups[1].Value;
        ...
        {
            if (ServiceManagementPattern.IsMatch(url))
                return RequestTarget.ServiceManagement;
            if (StoragePattern.IsMatch(url))
